...
export async function initStore() {
  ...
  // 启动阶段不可删除未被当前内存状态引用的图片：持久化恢复/云端合并可能尚未完成。
  // 删除仅在明确的任务/图片操作中执行，并由 deleteImageIfUnreferenced 二次确认。
  const imageIds = await getAllImageIds()
  scheduleThumbnailBackfill(imageIds.filter((id) => referencedIds.has(id)))

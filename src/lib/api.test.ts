...
import { DEFAULT_SETTINGS, getCustomProviderDefinition } from './apiProfiles'
...
describe('callImageApi', () => {
  ...
  it('uses the locked same-origin proxy for built-in sub2api submit and polling', async () => {
    vi.stubEnv('VITE_API_PROXY_AVAILABLE', 'true')
    vi.stubEnv('VITE_API_PROXY_LOCKED', 'true')
    const fetchMock = vi.spyOn(globalThis, 'fetch')
      .mockResolvedValueOnce(new Response(JSON.stringify({ task_id: 'imgtask-proxy' }), {
        status: 202,
        headers: { 'Content-Type': 'application/json' },
      }))
      .mockResolvedValueOnce(new Response(JSON.stringify({
    ...
    await callImageApi({
      settings: {
        ...
        profiles: [{
          ...DEFAULT_SETTINGS.profiles[0],
          id: 'sub2api-proxy-profile',
          provider: 'sb2api-async',
          baseUrl: 'https://sub2api.example.com/v1',
          apiKey: 'test-key',
          apiProxy: false,
        }],
        activeProfileId: 'sub2api-proxy-profile',
      },
      prompt: 'prompt',
      params: { ...DEFAULT_PARAMS },
      inputImageDataUrls: [],
    ...
    expect(fetchMock.mock.calls.map(([url, init]) => [url, (init as RequestInit).method])).toEqual([
      ['/api-proxy/images/generations/async', 'POST'],
      ['/api-proxy/images/tasks/imgtask-proxy', 'GET'],
    ])
  })

  it('uses the locked same-origin proxy when resuming a queued sub2api task', async () => {
    vi.stubEnv('VITE_API_PROXY_AVAILABLE', 'true')
    vi.stubEnv('VITE_API_PROXY_LOCKED', 'true')
    const profile = {
      ...DEFAULT_SETTINGS.profiles[0],
      provider: 'sb2api-async' as const,
      baseUrl: 'https://sub2api.example.com/v1',
      apiKey: 'test-key',
      apiProxy: false,
    }
    const provider = getCustomProviderDefinition(DEFAULT_SETTINGS, 'sb2api-async')!
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({
      status: 'completed',
      result: { data: [{ b64_json: 'aW1hZ2U=' }] },
    }), {
    ...
    await getCustomQueuedImageResult(profile, provider, 'imgtask-resumed', { ...DEFAULT_PARAMS })

    expect(fetchMock).toHaveBeenCalledWith(
      '/api-proxy/images/tasks/imgtask-resumed',

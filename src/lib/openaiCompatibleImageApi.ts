...
async function pollCustomTaskResult(
  ...
  const proxyConfig = readClientDevProxyConfig()
  const useApiProxy = shouldUseApiProxy(profile.apiProxy, proxyConfig)
  if (useApiProxy && customProvider.id !== 'sb2api-async') {
...
export async function getCustomQueuedImageResult(
  ...
  const mime = MIME_MAP[params.output_format] || 'image/png'
  return pollCustomTaskResult(profile, customProvider, customProvider.poll, taskId, mime)
}
...
async function callCustomHttpImageApi(opts: CallApiOptions, profile: ApiProfile, customProvider: CustomProviderDefinition): Promise<CallApiResult> {
  ...
  try {
    ...
    if (useApiProxy && customProvider.id !== 'sb2api-async' && (submitMapping.taskIdPath || customProvider.poll)) {
      throw new Error('API 代理暂不支持使用异步任务的自定义服务商。请关闭 API 代理，或改用同步返回图片的自定义服务商配置。')
    }
    ...
    return pollCustomTaskResult(profile, customProvider, customProvider.poll, taskId, mime, controller.signal)
  } finally {
